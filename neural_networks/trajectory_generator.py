...
class TrajectoryGenerator(object):
    ...
    def generate_trajectory(self,box_width=None, box_height=None, batch_size=None):
        ...
        for t in range(samples+1):
            ...
            if self.options.topology=='plane':
            ...
            # Traj on torus
            if self.options.topology=='torus':
                position[:,t+1,0] = np.mod(position[:,t+1,0] + box_width/2, box_width) - box_width/2
                position[:,t+1,1] = np.mod(position[:,t+1,1] + box_height/2, box_height) - box_height/2
        
            # Traj on Klein
            if self.options.topology=='klein':
                position[:,t+1,0] = np.mod(position[:,t+1,0] + box_width/2, box_width) - box_width/2
                for b in range(batch_size):
                    if abs(position[b,t+1,1])>=box_height/2:
                        position[b,t+1,1] = np.mod(position[b,t+1,1] + box_height/2, box_height) - box_height/2
                        position[b,t+1,0] = -position[b,t+1,0]
