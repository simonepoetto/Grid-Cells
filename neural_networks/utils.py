...
def load_model(options, model, folder=None):
    if not folder:
        folder = options.topology
    optimizer = tf.keras.optimizers.Adam(learning_rate=options.learning_rate)
    ckpt = tf.train.Checkpoint(step=tf.Variable(0), optimizer=optimizer, net=model)
    ckpt_dir='../trained_models/'+folder+'/'+options.run_ID+'/ckpts'
    ckpt_manager = tf.train.CheckpointManager(ckpt, ckpt_dir, max_to_keep=3)
    ckpt.restore(ckpt_manager.latest_checkpoint)
    if ckpt_manager.latest_checkpoint:
        print("Restored trained model from {}".format(ckpt_manager.latest_checkpoint))
    else:
        print("No trained model found")
    
    return model

def load_model_folder(options, model, folder):
    optimizer = tf.keras.optimizers.Adam(learning_rate=options.learning_rate)
    ckpt = tf.train.Checkpoint(step=tf.Variable(0), optimizer=optimizer, net=model)
    ckpt_dir=folder
    ckpt_manager = tf.train.CheckpointManager(ckpt, ckpt_dir, max_to_keep=3)
